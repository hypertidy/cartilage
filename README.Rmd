...
This is very much a work in progress. See functions `lamb` and `ambient`, which expect a raster. Be careful
not to input very large data (we'll add helpers), and keep dimensions to something like 300*300 or so (unless you keen). A local rgl device is highly recommended, rglwidget works fine but doesn't scale very well.  
...
set and settled on Monterey Bay. The original code was just to plot rayshader products in absolute coordinates, or to mesh other products into a rayshader scene. 
